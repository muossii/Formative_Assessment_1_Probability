...
The value given by the formula is different from the value given by the function. However, the values are close enough that the difference between them is almost negligible for descriptive purposes, but there might be a problem when applying the/basing on the values on statistical tests
